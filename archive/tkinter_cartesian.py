...
class TkinterCartesian():
    ...
    def plotgrid(self, gridlines=False, tick=10, mark=100):
        ...
        if self.CENTER:
            ...
            for i in range(-int(self.gridx/2), +int(self.gridx/2)):
                ticksize=0
                if i % tick == 0:
                    ticksize = 2
                ...
                if ticksize>0:
                    self.drawline((i,+ticksize), (i, -ticksize),
                                   color='grey', width=1)

            for i in range(-int(self.gridy/2), +int(self.gridy/2)):
                ticksize=0
                if i % tick == 0:
                    ticksize = 2
                ...
                if ticksize>0:
                    self.drawline((+ticksize, i), (-ticksize, i),
                                   color='grey', width=1)
    ...
    def xaxis(self, tick=10, mark=100, side='bottom'):
        '''  draw xaxis '''
        self.axw = Canvas(self.xaxisframe, width=self.aframe[0],
                          height=self.axiswidth, bg='black', bd=0)
        self.axw.pack()
        if self.CENTER:
            start, end = -int(self.gridx/2), +int(self.gridx/2)
        else:
            start, end = 0, self.gridx

        for i in range(start, end):
            point = (self._cartesian((i, 0)))
            ticksize=0
            if i % tick == 0:
                ticksize = 2
            ...
            if ticksize>0:
                self.axw.create_line((point[0]*self.cellx, 1),
                                     (point[0]*self.cellx, 1+ticksize*self.celly),
                                     fill='white', width=1)

    def yaxis(self, tick=10, mark=100, side='bottom'):
        '''  draw yaxis '''
        self.ayw = Canvas(self.yaxisframe, width=self.axiswidth,
                          height=self.aframe[1], bg='black', bd=0)
        self.ayw.pack()
        if self.CENTER:
            start, end = -int(self.gridy/2), +int(self.gridy/2)
        else:
            start, end = 0, self.gridy

        for i in range(start, end):
            point = (self._cartesian((0, i)))
            ticksize=0
            if i % tick == 0:
                ticksize = 2
            ...
            if ticksize>0:
                self.ayw.create_line((self.axiswidth+1, point[1]*self.celly),
                                     (self.axiswidth+1-ticksize*self.cellx,
                                     point[1]*self.celly),
                                     fill='white', width=1)
